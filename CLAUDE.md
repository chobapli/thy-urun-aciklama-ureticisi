...
- **Ürünler** → Firestore `products` koleksiyonu (gerçek zamanlı `onSnapshot`)
...
| `useCategories` | localStorage okuma/yazma, 300ms debounce |
...
- **Import** (`parseCSV`): `kod, kategori, alt_kategori, ad, görsel_url` başlıkları. Kategori/alt kategori adı Firestore id'ye dönüştürülür.
- **Export** (`exportCSV`): BOM ile UTF-8, `thy-urun-YYYY-MM-DD.csv` olarak indirilir.

### Firebase

- Proje ID: `tk-store-icerik`
- Hosting `dist/` klasörünü sunar, SPA için tüm yollar `index.html`'e yönlendirilir
- `db` ve `storage` `src/lib/firebase.ts`'den export edilir
